...
def manual(x):
    ...
    u_win = open("users_win.txt", "r")
    wins = u_win.readlines()
    wins[x-1] = str(users[x-1].get_wins())+"\n"
    print(wins[x-1])
    u_win = open("users_win.txt", "w")
    u_win.writelines(wins)
    u_win.close()
    games = u_games.readlines()
    users[x-1].set_games(users[x-1].get_games()+1)
    games[x-1] = str(users[x-1].get_games())+"\n"
    u_games = open("users_game.txt", "w")
    u_games.writelines(games)
    u_games.close()
...
def question(number_of_question, x):
    ...
    while q1.get_power() != str(power):
        q1 = questions[random.randint(0, len(questions)-1)]
    print("Your win is: ", win[number_of_question])
    ...
    if your_answer.upper() == q1.get_correct() and number_of_question!=7:
        ...
        if number_of_question in (1, 2, 3):
            number_of_question = 1
        elif number_of_question in (4, 5, 6, 7):
            number_of_question = 4
        users[x-1].set_wins(users[x-1].get_wins()+win[number_of_question])
        print("Congratulation! You've won ", win[number_of_question], "Now you have ", users[x-1].get_wins(), "$" )
        manual(x)

    elif  your_answer.upper() == q1.get_correct() and number_of_question==8:
        users[x-1].set_wins(users[x-1].get_wins()+win[number_of_question])
        print("Congratulation! You've won ", win[number_of_question],"You are the best!!!. Now you have ", users[x-1].get_wins(), "$" )
        manual(x)
    number_of_question = 1
...
def file_of_login(users):
    ...
    for login in u_login:
        if login.rstrip('\n') != "":
            ...
            games = int(u_games.readline().rstrip('\n'))
            wins = int(u_win.readline().rstrip('\n'))
...
def create_question(questions):
    ...
    q_power = open("question_powers.txt", "a")
    ...
    questions.append(Question(power, contents, correct, answers = [answerA, answerB, answerC, answerD]))
...
def file_of_question(questions):
    ...
    for contents in q_contents:
        if contents != "":
            ...
            questions.append(Question(power, contents, correct, [answerA, answerB, answerC, answerD]))
...
def get_account(users, number_of_question, questions):
    login = input("login: ")
    for y in range(1, len(users)+1):
        if login == users[y-1].get_login():
            password = input("password: ")
            if users[y-1].get_password() ==password:
                question(number_of_question, y)
        elif y == len(users) and login != users[y-1].get_login():
